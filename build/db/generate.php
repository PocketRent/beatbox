	...
	public function convertCode(CodeFile $f, string $dest, string $raw, bool $sub=false) {
		...
		if ($tstring == 'int' && $this->type_cat == PGType::TCAT_NUMERIC) {
			...
			$f->writeLine("\$__tmpArr_$hash = \db_parse_array('$subtype->delim',"
							. " substr($raw,1,-1));");
			$localVec = '$__localVec_'.$hash;
			$f->writeLine("$localVec = Vector {};");
			$f->writeLine("{$localVec}->reserve(count(\$__tmpArr_$hash));");
			$f->startBlock("foreach (\$__tmpArr_$hash as \$__elem_$hash)");
			$subtype->convertCode($f, "\$__convElem_$hash", "\$__elem_$hash", true);
			$f->writeLine("{$localVec}->add(\$__convElem_$hash);");
			$f->endBlock();
			$f->writeLine("$dest = $localVec;");
			...
			if ($this->type_cat == PGType::TCAT_USER_DEF) {
				$hash = substr(md5($dest.$raw.$this), 0, 6);
				switch ($this->name) {
				case 'hstore':
					$f->writeLine("\$__tmp_$hash = \db_parse_hstore($raw);");
					$f->writeLine("$dest = \$__tmp_$hash;");
					break;
				default:
					$f->writeLine("$dest = $raw;");
				}
	...
	public function writeTo(CodeFile $file) : void {
		...
		if ($this->type == PGType::T_COMPOSITE) {
			...
			foreach ($elements as $name => $type) {
				$file->writeLine("private ?$type \$_$name;");
			}
			$file->writeLine();
			...
			foreach ($elements as $name => $type) {
				$file->startBlock("public function get$name() : $type");
				$file->writeLine("return nullthrows(\$this->_$name);");
				$file->endBlock();

				$file->writeLine();

				$file->startBlock("public function set$name($type \$val) : this");
	...
	public function __toString(): string {
		if ($this->type == PGType::T_BASE) {
			if ($this->type_cat == PGType::TCAT_ARRAY) {
				...
				return "\\beatbox\\orm\\geom\\".$this->name;
...
function generate_php(Vector<Table> $tables, TypeDict $dict, string $directory,
	...
	foreach ($tables as $table) {
		...
		foreach ($table->columns as $col) {
			...
			$tbl_data->writeLine("private ?$type \$_$col->name = null;");
		}
		...
		foreach ($table->columns as $col) {
			...
			if ($col->nullable)
				...
				$tbl_data->writeLine("return nullthrows(\$this->_$col->name);");
			...
			if ($col->updatable) {
				if($isDateTime) {
					...
					$tbl_data->writeLine('$val = new \beatbox\orm\DateTimeType(\'@\' . intval($val));');
					$tbl_data->endBlock();
					$tbl_data->startBlock('if ($this->orig == null)');
					$tbl_data->writeLine("\$this->changed['$col->name'] = true;");
					$tbl_data->endBlock();
					$tbl_data->startBlock('else');
					$tbl_data->writeLine("\$orig = \$this->orig['$col->name'];");
					$tbl_data->writeLine("assert(\$orig instanceof \\beatbox\\orm\\DateTimeType);");
					$tbl_data->writeLine("\$this->changed['$col->name'] = \$orig->cmp(\$val) != 0;");
					$tbl_data->endBlock();
					$tbl_data->writeLine('assert($val instanceof \beatbox\orm\DateTimeType || '
											. '$val === null);');
					$tbl_data->writeLine("// UNSAFE");
					$tbl_data->writeLine("\$this->_$col->name = \$val;");
					$tbl_data->writeLine('return $this;');
				} else if ($type->isSimple()) {
					$tbl_data->startBlock("public function set$col->name($prefix$type_arg\$val$def_val): this");
					...
					$tbl_data->writeLine('return $this;');
					$tbl_data->endBlock();
					$tbl_data->startBlock("public function addAll$name({$prefix}Traversable<$subt> \$val$def_val): this");
					$tbl_data->writeLine("\$this->changed['$name'] = true;");
		...
		foreach ($table->columns as $col) {
			...
			$tbl_data->endBlock();
			$tbl_data->writeLine("\$orig['$col->name'] = \$this->_$col->name;");
		}
		$tbl_data->writeLine("\$this->orig = \$orig;");
		...
		$tbl_data->writeLine('$values = $values->map(inst_meth($con, \'escapeValue\'));');
		$tbl_data->writeLine('return \'ROW(\' . bb_join(\',\', $values) . \')\';');
		$tbl_data->endBlock();
		$tbl_data->writeLine();
		...
		$tbl_data->startBlock('return ImmSet {', '');
		foreach ($table->columns as $col) {
