 ...
 * @uses pr\base\errors\HTTP::error()
 */
function http_error(int $code, \string $status = null) {
	return pr\base\errors\HTTP::error($code, $status);
}
 ...
 * @uses pr\base\errors\HTTP::redirect()
 */
function redirect(string $to, $fallback=null) {
	return pr\base\errors\HTTP::redirect($to, $fallback);
}
 ...
 *
 * @param string $fallback the URL to fallback to
 * @uses pr\base\errors\HTTP::redirect_back()
 */
function redirect_back(string $fallback = null) {
	return pr\base\errors\HTTP::redirect_back($fallback);
}
